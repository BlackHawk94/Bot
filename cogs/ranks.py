...
class Ranks:
	...
	async def richest(self, ctx):
		...
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT "user", "name", "money" from profile ORDER BY "money" DESC LIMIT 10;')
		if ret==[]:
			return await ctx.send("No character have been created yet. Use `{ctx.prefix}create` to be the first one!")
		result = ""
		for profile in ret:
	...
	async def highscore(self, ctx):
		await ctx.trigger_typing()
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT "user", "name", "xp" from profile ORDER BY "xp" DESC LIMIT 10;')
		if ret==[]:
			return await ctx.send("No character have been created yet. Use `{ctx.prefix}create` to be the first one!")
		result = ""
		for profile in ret:
	...
	async def pvpstats(self, ctx):
		await ctx.trigger_typing()
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT "user", "name", "pvpwins" from profile ORDER BY "pvpwins" DESC LIMIT 10;')
		if ret==[]:
			return await ctx.send("No character have been created yet. Use `{ctx.prefix}create` to be the first one!")
		result = ""
		for profile in ret:
	...
	async def lovers(self, ctx):
		await ctx.trigger_typing()
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT "user", "marriage", "lovescore" FROM profile ORDER BY "lovescore" DESC LIMIT 10;')
		if ret == []:
			return await ctx.send("No character have been created yet. Use `{ctx.prefix}create` to be the first one!")
		result = ""
		for profile in ret:
		...
		result = discord.Embed(title="The Best lovers", description=result, colour=0xe7ca01)
		await ctx.send(embed=result)
