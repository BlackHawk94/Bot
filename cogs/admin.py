...
class Admin:
	...
	async def admingive(self, ctx, money: int, other: discord.User):
		...
		await ctx.send(f"Successfully gave **${money}** without a loss for you to **{other}**.")
	...
	async def adminrename(self, ctx, target: discord.User):
		...
		def mycheck(amsg):
			return amsg.author==ctx.author and len(amsg.content) < 21 and len(amsg.content) > 2
		try:
		...
		name = name.content
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET "name"=$1 WHERE "user"=$2;', name, target.id)
		channel = self.bot.get_channel(self.bot.config.admin_log_channel)
	...
	async def admincrate(self, ctx, target: discord.Member, amount: int=1):
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET "crates"="crates"+$1 WHERE "user"=$2;', amount, target.id)
		await ctx.send(f"Successfully gave **{amount}** crates to **{target}**.")
		channel = self.bot.get_channel(self.bot.config.admin_log_channel)
		await channel.send(f"**{ctx.author}** gave **{amount}** crates to **{target}**.")

	@is_admin()
	@commands.command(description="Gives XP to a user.")
	async def adminxp(self, ctx, target: discord.Member, amount: int):
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET "xp"="xp"+$1 WHERE "user"=$2;', amount, target.id)
