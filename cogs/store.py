...
class Store:
	...
	async def purchase(self, ctx, item:str, amount:int=1):
		...
		async with self.bot.pool.acquire() as conn:
			if item == 1:
				await conn.execute('UPDATE profile SET time_booster=time_booster+$1 WHERE "user"=$2;', amount, ctx.author.id)
			elif item == 2:
				await conn.execute('UPDATE profile SET luck_booster=luck_booster+$1 WHERE "user"=$2;', amount, ctx.author.id)
			elif item == 3:
				await conn.execute('UPDATE profile SET money_booster=money_booster+$1 WHERE "user"=$2;', amount, ctx.author.id)
	...
	async def boosters(self, ctx):
		async with self.bot.pool.acquire() as conn:
			boosters = await conn.fetchval('SELECT (time_booster, luck_booster, money_booster) FROM profile WHERE "user"=$1;', ctx.author.id)
			timeboosters = boosters[0]
			luckboosters = boosters[1]
			moneyboosters = boosters[2]
			active = []
			booster = ["`Time Booster`", "`Luck Booster`", "`Money Booster`"]
			active = await conn.fetch('SELECT "type", "end" FROM boosters WHERE "end" > clock_timestamp() AND "user"=$1;', ctx.author.id)
		nl = "\n"
		await ctx.send(embed=discord.Embed(title="Your Boosters", description=f"{'**Currently Active**'+nl if active!=[] else ''}{nl.join([booster[row[0]-1]+' active for another '+str(row[1]-datetime.datetime.now(datetime.timezone.utc)).split('.')[0] for row in active])}\n\nTime Boosters: `{timeboosters}`\nLuck Boosters: `{luckboosters}`\nMoney Boosters: `{moneyboosters}`", colour=discord.Colour.blurple()).set_footer(text=f"Use {ctx.prefix}activate to activate one"))

	@has_char()
	@commands.command(description="Uses a booster.")
	async def activate(self, ctx, boostertype: int):
		if not boostertype in range(1, 4):
			return await ctx.send("That is not a valid booster type. Must be from `1` to `3`.")
		booster = ["time_booster", "luck_booster", "money_booster"][boostertype-1]
		async with self.bot.pool.acquire() as conn:
			res = await conn.fetchval(f'SELECT {booster} FROM profile WHERE "user"=$1;', ctx.author.id)
			if res==0:
				return await ctx.send("You don't have any of these boosters.")
			check = await conn.fetchrow('SELECT * FROM boosters WHERE "type"=$1 AND "user"=$2;', boostertype, ctx.author.id)
			check2 = await conn.fetchrow('SELECT * FROM boosters WHERE "type"=$1 AND "user"=$2 AND clock_timestamp() > "end";', boostertype, ctx.author.id)
			if check and not check2:
				return await ctx.send(f"You already have one of these boosters active! Use `{ctx.prefix}boosters` to see how long it still lasts.")
			elif check and check2:
				return await conn.execute('DELETE FROM boosters WHERE "type"=$1 AND "user"=$2;', boostertype, ctx.author.id)
			await conn.execute(f'UPDATE profile SET {booster}={booster}-1 WHERE "user"=$1;', ctx.author.id)
			end = await conn.fetchval("SELECT clock_timestamp() + interval '1d';")
			await conn.execute('INSERT INTO boosters ("user", "type", "end") VALUES ($1, $2, $3);', ctx.author.id, boostertype, end)
