...
async def genstats(bot, userid, damage, armor):
	async with bot.pool.acquire() as conn:
		uclass = await conn.fetchval('SELECT class FROM profile WHERE "user"=$1;', userid)
...
def is_thief():
	async def predicate(ctx):
		async with ctx.bot.pool.acquire() as conn:
			ret = await conn.fetchval('SELECT class FROM profile WHERE "user"=$1;', ctx.author.id)
		if not ret:
			return False
		else:
...
class Classes:
	...
	async def _class(self, ctx, profession: str):
		...
		async with self.bot.pool.acquire() as conn:
			curclass = await conn.fetchval('SELECT class FROM profile WHERE "user"=$1;', ctx.author.id)
			if curclass == "No Class":
				await conn.execute('UPDATE profile SET "class"=$1 WHERE "user"=$2;', profession, ctx.author.id)
				await ctx.send(f"Your new class is now `{profession}`.")
			else:
				if not await has_money(self.bot, ctx.author.id, 5000):
					return await ctx.send(f"You're too poor for a class change, it costs **$5000**.")
				def check(m):
				...
				await conn.execute('UPDATE profile SET "class"=$1 WHERE "user"=$2;', profession, ctx.author.id)
				await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', 5000, ctx.author.id)
	...
	async def myclass(self, ctx):
		async with self.bot.pool.acquire() as conn:
			userclass = await conn.fetchval('SELECT class FROM profile WHERE "user"=$1;', ctx.author.id)
			if userclass == "No Class":
				await ctx.send("You haven't got a class yet.")
			else:
				try:
					await ctx.send(file=discord.File(f"classes/{userclass.lower().replace(' ', '_')}.png"))
				except:
					await ctx.send(f"The image for your class **{userclass}** hasn't been added yet.")
	...
	async def evolve(self, ctx):
		...
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET "class"=$1 WHERE "user"=$2;', newclass, ctx.author.id)
...
```""")
	...
	async def steal(self, ctx):
		...
		if random.randint(1,100) in range(1,grade*10+1):
			...
			async with self.bot.pool.acquire() as conn:
				user = await conn.fetchval('SELECT money FROM profile WHERE "user"=$1;', target.id)
				if not user:
					return await ctx.send(f"You tried to steal **{target}**, but that person doesn't have a character.")
				stolen = int(user*0.1)
				await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', stolen, ctx.author.id)
				await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', stolen, target.id)
