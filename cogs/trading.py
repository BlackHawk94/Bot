...
class Trading:
	...
	async def sell(self, ctx, itemid: int, price: int):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetchrow("SELECT * FROM inventory i JOIN allitems ai ON (i.item=ai.id) WHERE ai.id=$1 AND ai.owner=$2;", itemid, ctx.author.id)
			if not ret:
			...
			await conn.execute("DELETE FROM inventory i USING allitems ai WHERE i.item=ai.id AND ai.id=$1 AND ai.owner=$2;", itemid, ctx.author.id)
			await conn.execute("INSERT INTO market (item, price) VALUES ($1, $2);", itemid, price)
	...
	async def buy(self, ctx, itemid: int):
		async with self.bot.pool.acquire() as conn:
			item = await conn.fetchrow("SELECT * FROM market m JOIN allitems ai ON (m.item=ai.id) WHERE ai.id=$1;", itemid)
			if not item:
				return await ctx.send(f"There is no item in the shop with the ID: {itemid}")
			if not await has_money(self.bot, ctx.author.id, item[2]):
				return await ctx.send("You're too poor to buy this item.")
			deleted = await conn.fetchrow("DELETE FROM market m USING allitems ai WHERE m.item=ai.id AND ai.id=$1 AND ai.owner=$2 RETURNING *;", itemid, item[4])
			await conn.execute("UPDATE allitems SET owner=$1 WHERE id=$2;", ctx.author.id, deleted[3])
			await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', deleted[2], deleted[4])
			await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', deleted[2], ctx.author.id)
			await conn.execute("INSERT INTO inventory (item, equipped) VALUES ($1, $2);", deleted[3], False)
		...
		await seller.send(f"**{ctx.author}** bought your **{deleted['name']}** for **${deleted['price']}** from the market.")

	@has_char()
	@commands.command(description="Takes an item off the shop.")
	async def remove(self, ctx, itemid: int):
		async with self.bot.pool.acquire() as conn:
			item = await conn.fetchrow("SELECT * FROM market m JOIN allitems ai ON (m.item=ai.id) WHERE ai.id=$1 AND ai.owner=$2;", itemid, ctx.author.id)
			if not item:
				return await ctx.send(f"You don't have an item of yours in the shop with the ID `{itemid}`.")
			deleted = await conn.fetchrow("DELETE FROM market m USING allitems ai WHERE m.item=ai.id AND ai.id=$1 AND ai.owner=$2 RETURNING *;", itemid, ctx.author.id)
			await conn.execute("INSERT INTO inventory (item, equipped) VALUES ($1, $2);", itemid, False)
	...
	async def shop(self, ctx, itemtype:str="All", minstat:float=0.00, highestprice:int=10000):
		...
		async with self.bot.pool.acquire() as conn:
			if itemtype == "All":
				ret = await conn.fetch('SELECT * FROM allitems ai JOIN market m ON (ai.id=m.item) WHERE m."price"<$1 AND (ai."damage">=$2 OR ai."armor">=$3);', highestprice, minstat, minstat)
			elif itemtype == "Sword":
				ret = await conn.fetch('SELECT * FROM allitems ai JOIN market m ON (ai.id=m.item) WHERE ai."type"=$1 AND ai."damage">=$2 AND m."price"<$3;', itemtype, minstat, highestprice)
			elif itemtype == "Shield":
				ret = await conn.fetch('SELECT * FROM allitems ai JOIN market m ON (ai.id=m.item) WHERE ai."type"=$1 AND ai."armor">=$2 AND m."price"<$3;', itemtype, minstat, highestprice)
		if ret==[]:
	...
	async def offer(self, ctx, itemid: int, price: int, user:discord.Member):
		...
		try:
			...
			return await ctx.send(f"They didn't want to buy your item, {ctx.author.mention}.")
		...
		async with self.bot.pool.acquire() as conn:
			...
			await conn.execute("UPDATE allitems SET owner=$1 WHERE id=$2;", user.id, itemid)
			await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', price, ctx.author.id)
			await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', price, user.id)
			await conn.execute('UPDATE inventory SET "equipped"=$1 WHERE "item"=$2;', False, itemid)
	...
	async def merchant(self, ctx, itemid: int):
		async with self.bot.pool.acquire() as conn:
			item = await conn.fetchrow("SELECT * FROM inventory i JOIN allitems ai ON (i.item=ai.id) WHERE ai.id=$1 AND ai.owner=$2;", itemid, ctx.author.id)
			if not item:
				return await ctx.send(f"You don't own an item with the ID: {itemid}")
			await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', item[6], ctx.author.id)
			await conn.execute('DELETE FROM allitems WHERE "id"=$1;', itemid)
		await ctx.send(f"You received **${item[6]}** when selling item `{itemid}`.")

	@has_char()
	@commands.command(description="Sells all items except your equipped ones for their value.")
	async def merchall(self, ctx):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetchrow("SELECT sum(value), count(value) FROM inventory i JOIN allitems ai ON (i.item=ai.id) WHERE ai.owner=$1 AND i.equipped IS FALSE;", ctx.author.id)
			if ret[1] == 0:
				return await ctx.send("Nothing to merch.")
			await conn.execute('DELETE FROM allitems ai USING inventory i WHERE ai.id=i.item AND ai.owner=$1 AND i.equipped IS FALSE;', ctx.author.id)
			await conn.execute('UPDATE profile SET "money"="money"+$1 WHERE "user"=$2;', ret[0], ctx.author.id)
		await ctx.send(f"Merched **{ret[1]}** items for **${ret[0]}**.")

	@commands.command(description="Views your pending shop items.")
	async def pending(self, ctx):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT * FROM allitems ai JOIN market m ON (m.item=ai.id) WHERE ai."owner"=$1;', ctx.author.id)
		if ret == []:
...
""")
		...
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', item[5], ctx.author.id)
			itemid = await conn.fetchval('INSERT INTO allitems ("owner", "name", "value", "type", "damage", "armor") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', ctx.author.id, item[4], 0, item[0], item[1], item[2])
			await conn.execute('INSERT INTO inventory ("item", "equipped") VALUES ($1, $2);', itemid, False)
