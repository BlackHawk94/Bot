...
class Crates:
	...
	async def crates(self, ctx):
		async with self.bot.pool.acquire() as conn:
			crates = await conn.fetchval('SELECT crates FROM profile WHERE "user"=$1;', ctx.author.id)
	...
	async def _open(self, ctx):
		async with self.bot.pool.acquire() as conn:
			crates = await conn.fetchval('SELECT crates FROM profile WHERE "user"=$1;', ctx.author.id)
			if crates < 1:
				return await ctx.send("Seems you haven't got a crate yet. Vote me up to get some or earn them!")
			...
			await conn.execute('UPDATE profile SET crates=crates-1 WHERE "user"=$1;', ctx.author.id)
	...
	async def tradecrate(self, ctx, other:discord.Member):
		async with self.bot.pool.acquire() as conn:
			crates = await conn.fetchval('SELECT crates FROM profile WHERE "crates">0 AND "user"=$1;', ctx.author.id)
			if not crates:
				return await ctx.send("You don't have any crates.")
			await conn.execute('UPDATE profile SET crates=crates-1 WHERE "user"=$1;', ctx.author.id)
			await conn.execute('UPDATE profile SET crates=crates+1 WHERE "user"=$1;', other.id)
