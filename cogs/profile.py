...
class Profile:
	...
	async def create(self, ctx):
		...
		if len(name) > 2 and len(name) < 21:
			async with self.bot.pool.acquire() as conn:
				await conn.execute("INSERT INTO profile VALUES ($1, $2, $3, $4);", ctx.author.id, name, 100, 0)
				itemid = await conn.fetchval("INSERT INTO allitems (owner, name, value, type, damage, armor) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id;", ctx.author.id, "Starter Sword", 0, "Sword", 3.0, 0.0)
				await conn.execute("INSERT INTO inventory (item, equipped) VALUES ($1, $2);", itemid, True)
				itemid = await conn.fetchval("INSERT INTO allitems (owner, name, value, type, damage, armor) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id;", ctx.author.id, "Starter Shield", 0, "Shield", 0.0, 3.0)
				await conn.execute("INSERT INTO inventory (item, equipped) VALUES ($1, $2);", itemid, True)
	...
	async def profile(self, ctx, *, person: discord.User=None):
		...
		async with self.bot.pool.acquire() as conn:
			ranks = []
			profile = await conn.fetchrow('SELECT * FROM profile WHERE "user"=$1;', targetid)
			color = profile["colour"]
			sword = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Sword';", targetid)
			shield = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Shield';", targetid)
			ranks.append((await conn.fetchrow("SELECT position FROM (SELECT profile.*, ROW_NUMBER() OVER(ORDER BY profile.money DESC) AS position FROM profile) s WHERE s.user = $1 LIMIT 1;", targetid))[0])
			ranks.append((await conn.fetchrow("SELECT position FROM (SELECT profile.*, ROW_NUMBER() OVER(ORDER BY profile.xp DESC) AS position FROM profile) s WHERE s.user = $1 LIMIT 1;", targetid))[0])
			mission = await conn.fetchrow('SELECT * FROM mission WHERE "name"=$1;', targetid)
			guild = await conn.fetchrow('SELECT name FROM guild WHERE "id"=$1;', profile[12])
			if not mission:
				mission = []
			missionend = []
			if mission != []:
				missionend = (await conn.fetchrow('SELECT $1-clock_timestamp();', mission[2]))[0]
	...
	async def xp(self, ctx):
		async with self.bot.pool.acquire() as conn:
			points = await conn.fetchval('SELECT xp FROM profile WHERE "user"=$1;', ctx.author.id)
	...
	async def inventory(self, ctx):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch("SELECT ai.*, i.equipped FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE p.user=$1 ORDER BY i.equipped DESC;", ctx.author.id)
		if ret == []:
	...
	async def equip(self, ctx, itemid: int):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetch('SELECT ai.id FROM inventory i JOIN allitems ai ON (i.item=ai.id) WHERE ai.owner=$1;', ctx.author.id)
			if not ret:
				return await ctx.send("Your inventory is empty.")
			ids = [r[0] for r in ret]
			if itemid in ids:
				itemtype = await conn.fetchval('SELECT type FROM allitems WHERE "id"=$1;', itemid)
				olditem = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type=$2;", ctx.author.id, itemtype)
				if olditem is not None:
					await conn.execute('UPDATE inventory SET "equipped"=False WHERE "item"=$1;', olditem[0])
				await conn.execute('UPDATE inventory SET "equipped"=True WHERE "item"=$1;', itemid)
				if olditem is not None:
	...
	async def merge(self, ctx, firstitemid: int, seconditemid: int):
		...
		async with self.bot.pool.acquire() as conn:
			item = await conn.fetchrow('SELECT * FROM allitems WHERE "id"=$1 AND "owner"=$2;', firstitemid, ctx.author.id)
			item2 = await conn.fetchrow('SELECT * FROM allitems WHERE "id"=$1 AND "owner"=$2;', seconditemid, ctx.author.id)
			if not item and item2:
			...
			if stat2[1] < stat1[1] - 5 or stat2[1] > stat1[1] + 5:
				return await ctx.send(f"The seconds item's stat must be in the range of `{stat1[1] - 5}` to `{stat1[1] + 5}` to upgrade an item with the stat of `{stat1[1]}`.")
			if stat1[1] > 40:
				return await ctx.send("This item is already on the maximum upgrade level.")
			await conn.execute(f'UPDATE allitems SET {stat1[0]}={stat1[0]}+1 WHERE "id"=$1;', firstitemid)
			await conn.execute('DELETE FROM allitems WHERE "id"=$1;', seconditemid)
		await ctx.send(f"The {stat1[0]} of your **{item['name']}** is now **{stat1[1] + 1}**. The other item was destroyed.")
	...
	async def upgradeweapon(self, ctx, itemid: int):
		async with self.bot.pool.acquire() as conn:
			item = await conn.fetchrow('SELECT * FROM allitems WHERE "id"=$1 AND "owner"=$2;', itemid, ctx.author.id)
			if not item:
			...
			usermoney = (await conn.fetchrow('SELECT money FROM profile WHERE "user"=$1;', ctx.author.id))[0]
		...
		ctx.command.reset_cooldown(ctx)
		async with self.bot.pool.acquire() as conn:
			await conn.execute(f'UPDATE allitems SET {stattoupgrade}={stattoupgrade}+1 WHERE "id"=$1;', itemid)
			await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', pricetopay, ctx.author.id)
	...
	async def give(self, ctx, money: int, other: discord.Member):
		...
		async with self.bot.pool.acquire() as conn:
			...
			await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', money, other.id)
	...
	async def rename(self, ctx):
		...
		try:
			...
			if len(name)>2 and len(name) < 21:
				async with self.bot.pool.acquire() as conn:
					await conn.execute('UPDATE profile SET "name"=$1 WHERE "user"=$2;', name, ctx.author.id)
	...
	async def colour(self, ctx, colour: str):
		...
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET "colour"=$1 WHERE "user"=$2;', colour, ctx.author.id)
