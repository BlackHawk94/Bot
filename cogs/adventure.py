...
class Adventure:
	...
	async def adventures(self, ctx):
		async with self.bot.pool.acquire() as conn:
			...
			sword = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Sword';", ctx.author.id)
			shield = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Shield';", ctx.author.id)
			playerxp = await conn.fetchval('SELECT xp FROM profile WHERE "user"=$1;', ctx.author.id)
	...
	async def adventure(self, ctx, dungeonnumber: int):
		if dungeonnumber > 15 or dungeonnumber < 1:
			return await ctx.send("Enter a number from **1** to **15**.")
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetchrow('SELECT * FROM mission WHERE "name"=$1;', ctx.author.id)
			if ret:
			...
			boostertest = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 1)
			boostertest2 = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2 AND clock_timestamp() < "end";', ctx.author.id, 1)
			if not boostertest and not boostertest2:
				end = await conn.fetchval("SELECT clock_timestamp() + $1::interval;", todelta(times[dungeonnumber]))
			elif boostertest and not boostertest2:
				await conn.execute('DELETE FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 1)
				end = await conn.fetchval("SELECT clock_timestamp() + $1::interval;", todelta(times[dungeonnumber]))
			elif boostertest and boostertest2:
				end = await conn.fetchval("SELECT clock_timestamp() + $1::interval;", todelta(booster_times[dungeonnumber]))
			await conn.execute('INSERT INTO mission ("name", "end", "dungeon") VALUES ($1, $2, $3);', ctx.author.id, end, dungeonnumber)
	...
	async def activeadventure(self, ctx):
		async with self.bot.pool.acquire() as conn:
			current = await conn.fetchrow('SELECT * FROM mission WHERE "name"=$1;', ctx.author.id)
			if current:
...
""", delete_after=10)
			...
			if move == "attack" and enemymove == "attack":
				...
				HP -= efficiency
				ENEMY_HP -= SWORD
				await ctx.send(f"You hit the enemy for **{SWORD}** damage, he hit you for **{efficiency}** damage.", delete_after=10)
			elif move == "attack" and enemymove != "defend":
				ENEMY_HP -= SWORD
				await ctx.send(f"You hit the enemy for **{SWORD}** damage.", delete_after=10)
			elif enemymove == "attack" and move == "recover":
				efficiency = random.randint(int(SWORD*0.5), int(SWORD*1.5))
				HP -= efficiency
				await ctx.send(f"The enemy hit you for **{efficiency}** damage.", delete_after=10)
			if ENEMY_HP < 1:
		...
		async with self.bot.pool.acquire() as conn:
			if shieldorsword == "Sword":
				...
				item = await conn.fetchrow('INSERT INTO allitems ("owner", "name", "value", "type", "damage", "armor") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', ctx.author.id, itemname, itemvalue, "Sword", maximumstat, 0.00)
			elif shieldorsword == "Shield":
				itemname = random.choice(names)+random.choice([" Shield", " Defender", " Aegis"])
				item = await conn.fetchrow('INSERT INTO allitems ("owner", "name", "value", "type", "damage", "armor") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', ctx.author.id, itemname, itemvalue, "Shield", 0.00, maximumstat)
			await conn.execute('INSERT INTO inventory ("item", "equipped") VALUES ($1, $2);', item[0], False)
	...
	async def status(self, ctx):
		async with self.bot.pool.acquire() as conn:
			ret = await conn.fetchrow('SELECT * FROM mission WHERE "name"=$1;', ctx.author.id)
			if not ret:
				return await ctx.send(f"You are on no mission yet. Use `{ctx.prefix}adventure [DungeonID]` to go out on an adventure!")
			isfinished = await conn.fetchrow('SELECT * FROM mission WHERE name=$1 AND clock_timestamp() > "end";', ctx.author.id)
			if isfinished:
				sword = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Sword';", ctx.author.id)
				shield = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Shield';", ctx.author.id)
				playerxp = await conn.fetchval('SELECT xp FROM profile WHERE "user"=$1;', ctx.author.id)
				...
				boostertest = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 2)
				boostertest2 = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2 AND clock_timestamp() < "end";', ctx.author.id, 2)
				if not boostertest and not boostertest2:
					success = rpgtools.calcchance(swordbonus, shieldbonus, isfinished[3], int(playerlevel), returnsuccess=True)
				elif boostertest and not boostertest2:
					await conn.execute('DELETE FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 2)
					success = rpgtools.calcchance(swordbonus, shieldbonus, isfinished[3], int(playerlevel), returnsuccess=True)
				elif boostertest and boostertest2:
				...
				if success:
					...
					boostertest = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 3)
					boostertest2 = await conn.fetchval('SELECT "end" FROM boosters WHERE "user"=$1 AND "type"=$2 AND clock_timestamp() < "end";', ctx.author.id, 3)
					if not boostertest and not boostertest2:
						gold = random.randint(1,30)*isfinished[3]
					elif boostertest and not boostertest2:
						await conn.execute('DELETE FROM boosters WHERE "user"=$1 AND "type"=$2;', ctx.author.id, 3)
						gold = random.randint(1,30)*isfinished[3]
					elif boostertest and boostertest2:
					...
					if shieldorsword == "sw":
						item = await conn.fetchrow('INSERT INTO allitems ("owner", "name", "value", "type", "damage", "armor") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', ctx.author.id, random.choice(names)+random.choice([" Sword", " Blade", " Stich"]), random.randint(1,40)*isfinished[3], "Sword", maximumstat, 0.00)
					if shieldorsword == "sh":
						item = await conn.fetchrow('INSERT INTO allitems ("owner", "name", "value", "type", "damage", "armor") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;', ctx.author.id, random.choice(names)+random.choice([" Shield", " Defender", " Aegis"]), random.randint(1,40)*isfinished[3], "Shield", 0.00, maximumstat)
					await conn.execute('INSERT INTO inventory ("item", "equipped") VALUES ($1, $2);', item[0], False)
					#marriage partner should get 50% of the money
					partner = await conn.fetchval('SELECT marriage FROM profile WHERE "user"=$1;', ctx.author.id)
					if partner != 0:
						await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', int(gold/2), partner)
					#guild money
					guild = await conn.fetchval('SELECT guild FROM profile WHERE "user"=$1;', ctx.author.id)
					if guild != 0:
						await conn.execute('UPDATE guild SET money=money+$1 WHERE "id"=$2;', int(gold/10), guild)

					await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', gold, ctx.author.id)
					await conn.execute('UPDATE profile SET xp=xp+$1 WHERE "user"=$2;', xp, ctx.author.id)
					await conn.execute('UPDATE profile SET completed=completed+1 WHERE "user"=$1;', ctx.author.id)
					if partner == 0:
					...
					await conn.execute('UPDATE profile SET deaths=deaths+1 WHERE "user"=$1;', ctx.author.id)
				...
				dungeon = await conn.fetchrow("SELECT * FROM dungeon WHERE id=$1;", mission[3])
	...
	async def deaths(self, ctx):
		async with self.bot.pool.acquire() as conn:
			stats = await conn.fetchrow('SELECT deaths, completed FROM profile WHERE "user"=$1;', ctx.author.id)
