...
class Tournament:
	...
	async def tournament(self, ctx, prize:int):
		...
		while len(remain) > 1:
			...
			for match in matches:
				...
				async with self.bot.pool.acquire() as conn:
					sword1 = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Sword';", match[0].id)
					try:
						sw1 = sword1[5]
					except:
						sw1 = 0
					shield1 = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Shield';", match[0].id)
					try:
						sh1 = shield1[6]
					except:
						sh1 = 0
					sword2 = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Sword';", match[1].id)
					try:
						sw2 = sword2[5]
					except:
						sw2 = 0
					shield2 = await conn.fetchrow("SELECT ai.* FROM profile p JOIN allitems ai ON (p.user=ai.owner) JOIN inventory i ON (ai.id=i.item) WHERE i.equipped IS TRUE AND p.user=$1 AND type='Shield';", match[1].id)
					try:
						sh2 = shield2[6]
					except:
						sh2 = 0
		...
		async with self.bot.pool.acquire() as conn:
			await conn.execute('UPDATE profile SET money=money-$1 WHERE "user"=$2;', prize, ctx.author.id)
			await conn.execute('UPDATE profile SET money=money+$1 WHERE "user"=$2;', prize, remain[0].id)
